                ...
                <div class="card">
                    <div class="card-content">
                        <div class="card-body">
                            ...
                            <form action="{{ route('admin.roles.update', $role) }}" class="form" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <div class="row">
                                    <div class="col-md-3 col-12">
                                        @input(['name' => 'name', 'label' => 'Название', 'entity' => $role])
                                    </div>
                                        ...
                                        <fieldset class="form-group">
                                            ...
                                            <select class="form-select choices multiple-remove" id="permissions" name="permissions[]" multiple>
                                                ...
                                                @foreach($permissions as $permission)
                                                    <option value="{{ $permission->id }}" @if (in_array($permission->id, $role->permissions->pluck('id')->toArray(), true)) selected @endif>
                                                        {{ $permission->name }}
                                                    </option>
                                                @endforeach
                                                </optgroup>
